...
var heal_fatigue_turns: int = 0 # Tracks the 5-turn freeze
...
func _ready():
	...
	modulate = Color(0.8, 0.2, 0.8) # Purple
	# Snap to grid center
...
func _on_player_turn_finished():
	...
	# 1. HANDLE FATIGUE: If we recently healed, we are frozen for 5 turns
	if heal_fatigue_turns > 0:
		heal_fatigue_turns -= 1
		modulate.a = 0.4 # Ghostly/Exhausted look
		return 
	
	# 2. ACTIVE STATE: Normal behavior
	modulate.a = 1.0 
	turn_counter += 1
	
	# 3. HEAL TRIGGER: Every 3 active turns
	if turn_counter >= 3:
		turn_counter = 0
		execute_heal_pulse()
		heal_fatigue_turns = 5 # Set the 5-turn freeze after healing
		return 
	
	# 4. MOVE LOGIC: Diagonal kiting toward a "Sweet Spot"
...
func process_diagonal_kiting():
	...
	var ideal_dist_px = grid_size * 3.5 # The "Sweet Spot" range
	var best_score = 999999.0 

	for dir in directions:
		# Bishop can slide up to 4 tiles diagonally
		for distance in range(1, 3):
			...
			var score = abs(d_to_p - ideal_dist_px) # Find tile closest to 3.5 grid distance
...
func execute_heal_pulse():
	...
	# Visual: Bishop Pulses Intense Green
	var tween = create_tween()
	tween.tween_property(self, "modulate", Color(0, 10, 0), 0.2) 
	await tween.finished
	
	# Logic: Heal all allies in group "enemies" within 6 tiles
	...
	# Reset Visuals and set starting fatigue transparency
	var reset = create_tween()
	reset.tween_property(self, "modulate", original_color, 0.3)
	await reset.finished
	
	modulate.a = 0.4 # Fade out immediately to show fatigue
	is_moving = false

func flash_target_green(target):
	var old_color = target.modulate
	target.modulate = Color(0, 10, 0) # Glowing Green
	# Non-blocking timer to return ally color to normal
